...
select max(timestamp) into max_time from ISBDBA.AUDIT_DDL_SUCCESS@LINK_TO_EMREP where database_name = dbname;

INSERT INTO ISBDBA.AUDIT_DDL_SUCCESS@LINK_TO_EMREP
SELECT   DBNAME,
   ...
   WHERE TIMESTAMP > max_time
     ...
     AND NOT (    UPPER (os_username) IN ('ORACLE', 'PERF')
              AND (UPPER (userhost) IN (SELECT host_name
                                          FROM gv$instance))
             )
     AND returncode = 0
